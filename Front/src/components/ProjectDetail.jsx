...
// MODIFICA: usa direttamente i progetti da localStorage (dove vengono salvati nella pagina Portfolio)
const TECH_ICONS = {
  React: Globe,
  Tailwind: Layout,
  Express: Cpu,
    ...
    <div className="grid grid-cols-2 gap-3 md:gap-4 p-3 md:p-4 bg-[#0a0a1a] rounded-xl overflow-hidden relative">
      ...
      <div className="relative z-10 flex items-center space-x-2 md:space-x-3 bg-white/5 p-2 md:p-3 rounded-lg border border-purple-500/20 transition-all duration-300 hover:scale-105 hover:border-purple-500/50 hover:shadow-lg">
        ...
        <div className="flex-grow">
          ...
          <div className="text-[10px] md:text-xs text-gray-400">level Count</div>
        </div>
      </div>
    </div>
  );
...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    ...
    const selectedProject = storedProjects.find((p) => String(p.id) === id || p.id === id);
    if (selectedProject) {
      const enhancedProject = {
        ...selectedProject,
        Features: selectedProject.Features || [],
        TechStack: selectedProject.TechStack || [],
        Github: selectedProject.Github || 'https://github.com/salvo1992',
      };
